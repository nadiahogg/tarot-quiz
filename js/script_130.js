...
function newQuestion() {
    ...
    randomQuestion = quiz.questions[Math.floor(Math.random() * quiz.questions.length)];
    displayedText = randomQuestion.text;
    answerOptions = randomQuestion.options;
    questionIndex = quiz.questions.indexOf(randomQuestion);
    askedQuestion = quiz.questions.splice(questionIndex, 1);
    currentQuestionNumber = quiz.currentQuestion + 1;
	...
	if (guess == quiz.questions[quiz.currentQuestion].correct) {
    console.log("correct!");
  } else {
